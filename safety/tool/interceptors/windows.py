...
class WindowsInterceptor(CommandInterceptor):
    ...
    def __get_powershell_profiles(self) -> Dict[str, Path]:
        ...
        for shell, shell_name in shells:
            try:
                ...
                try:
                    ...
                    LOG.info(f"{shell_name} not found, skipping profile setup")
                    continue
                ...
                if result.returncode == 0 and result_stdout:
                    ...
                    LOG.info(f"Found {shell_name} profile at {profile_path}")
                else:
                    LOG.info(
                        f"Failed to get {shell_name} profile path: {result.stderr.strip()}"
                    )
            except Exception as e:
                LOG.info(f"Error while getting {shell_name} profile: {str(e)}")
        ...
        if not profiles:
            ...
            LOG.info(f"Using fallback profile at {default_path}")
        ...
        for _, profile_path in profiles.items():
            try:
                ...
                if self.marker_start not in profile_content:
                    ...
                    LOG.info(f"Added PowerShell wrapper to {profile_path}")
            except Exception as e:
                LOG.info(
        ...
        for _, profile_path in profiles.items():
            try:
                if profile_path.exists():
                    ...
                    if self.marker_start not in profile_content:
                        LOG.info(f"PowerShell wrapper not found in {profile_path}")
                        continue
                    ...
                    LOG.info(f"Removed PowerShell wrapper from {profile_path}")
            except Exception as e:
                LOG.info(
